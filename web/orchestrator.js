...
function matchesExpected(actual, expected) {
  ...
  return Object.entries(expectedArgs).every(([key, value]) => actualArgs[key] === value);
}

async function runModelCases(config, modelName, cases, options = {}) {
  const state = createOrchestratorState(modelName);
  const caseResults = [];
  const perTurnLatencies = [];

  for (const benchCase of cases) {
    resetOrchestratorState(state, modelName);
    const turnDetails = [];
    let lastTurnResult = null;

    for (const utterance of benchCase.dialogue) {
      lastTurnResult = await processUtterance(state, utterance, {
        baseUrl: config.baseUrl,
        apiKey: config.apiKey,
        model: modelName,
      }, options);

      perTurnLatencies.push(lastTurnResult.latencyMs);
      turnDetails.push({
        utterance,
        assistantText: lastTurnResult.assistantText,
        functionCall: deepClone(lastTurnResult.functionCall),
        latencyMs: lastTurnResult.latencyMs,
        error: lastTurnResult.error,
      });
    }

    const finalCall = deepClone(state.lastFunctionCall);
    const passed = matchesExpected(finalCall, benchCase.expected);

    caseResults.push({
      name: benchCase.name,
      dialogue: benchCase.dialogue,
      expected: benchCase.expected,
      actual: finalCall,
      passed,
      latencyMs: lastTurnResult ? lastTurnResult.latencyMs : 0,
      turns: turnDetails,
  ...
  const avgLatencyMs =
    perTurnLatencies.length === 0
      ? 0
      : perTurnLatencies.reduce((acc, value) => acc + value, 0) / perTurnLatencies.length;
  ...
  return {
    ...
    summary: {
      ...
      accuracy: caseResults.length ? passedCount / caseResults.length : 0,
      avgLatencyMs,
    },
    cases: caseResults,
  };
...
export async function runBenchmark(config, cases, options = {}) {
  const selectedCases = cases || [];

  const baseRun = await runModelCases(config, config.baseModel, selectedCases, options);
  const distilRun = await runModelCases(config, config.distilModel, selectedCases, options);

  const comparison = {
    passCountDelta: distilRun.summary.passedCases - baseRun.summary.passedCases,
    accuracyDelta: distilRun.summary.accuracy - baseRun.summary.accuracy,
    avgLatencyDeltaMs: distilRun.summary.avgLatencyMs - baseRun.summary.avgLatencyMs,
  };
...
export function downloadJsonReport(report) {
  ...
  anchor.href = url;
  anchor.download = `benchmark-report-${timestamp}.json`;
